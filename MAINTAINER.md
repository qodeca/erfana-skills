...
- **Plugin scope**: design + orchestration toolkit (since v4.0.0; process skill `grill-me` added v4.2.3; verification skill `fact-checking` added v4.2.7; researched review command `lens-review` added v4.2.11; PM/PO issue-translation command `explain-issue` added v4.2.14; skill-scoped Stop hooks added v6.2.0 (`grill-me/grill-guard`) and post-6.3.0 (`managing-skills/ms-grill-guard`); managing-skills coverage-map requirements interviews via the shared `grill-planner` agent post-6.3.0). 15 skills + 87 shared agents + per-skill internal agents under `skills/managing-reports/agents/` + 4 safety hooks + 5 slash commands. Bus-factor risk doubled vs. the v3.x design-only era; backup-maintainer onboarding remains an active priority. Live counts are enforced by Gate 15 (`scripts/gate-15-doc-claims.sh`) – this paragraph is prose, the source of truth is the filesystem.
...
- **Brand-system smoke checks not yet in checklist**: the v2.3 brand-system layer adds runtime concerns (mid-session `ACTIVE_BRAND` swap reflected in renders, broken-fixture rejection by Gate 12) that the pre-release smoke checklist above does not yet cover. Tracked in `ROADMAP.md` under `## v2.5.0 – explicit exit condition + bus-factor` (the former issue link was recycled by the fresh-repo publish – GitHub #18 is now an unrelated merged PR) – when that lands, this section can be deleted.
