...
- **BREAKING: the six `design-*` skills are gone.** `design-direction`, `design-prototype`, `design-slides`, `design-motion`, `design-infographic` and `design-review` were unused in practice and are removed. `/erfana:design-*` no longer resolves. There is no migration path and no deprecation window inside this plugin – a user who needs design output should install a design-focused plugin.
- **BREAKING: `skills/design-shared/` is gone** – the ~31 MB shared bundle (assets, demos, export scripts, cross-cutting references, `test-prompts.json`) and the entire brand-system layer under `brands/` (manifests, DTCG token files, logo SVGs, the brand schema, `ACTIVE_BRAND`). Anything that read `../design-shared/...` was removed with it.
- **Six verification gates retired** with the surface they guarded: Gate 5 (SVG/HTML well-formedness + SVG content safety), Gate 6 (JSX balance), Gate 8 (design trigger-phrase coverage), Gate 9 (watermark consistency), Gate 12 (brand manifests) and Gate 13 (brandbook hex coverage, the only soft gate). `scripts/gate-12-brand-manifests.sh`, `scripts/check-brandbook-hex.sh`, `scripts/_lib/brandbook-hex-inventory.json` and the six `docs/gates/` detail files were deleted. The suite is now **12 gates, all hard**. Surviving gates keep their original numbers, so the numbering is deliberately non-contiguous – older commits, CI logs and caveat entries stay readable.
- **Five `tests/design-*` scratch folders** removed. `tests/` now holds `gate-02-fixtures/` and `hooks/` (both replayed by CI) plus `managing-skills/v4.2.0-pilot.md`, a historical pilot record whose subject skill was deleted in this release.
- **`scripts/_lib/json_schema_lite.py`** removed – 165 lines of hand-rolled schema validation whose only consumer was the deleted brand-manifest gate. Its Gate 4 syntax check went with it, so Gate 4 now covers `gate2_detector.py` (previously unchecked) instead.

### Changed

- Gates 3, 4 and 7 narrowed to the surviving tree (no `design-shared` JSON, scripts or brand prose in their globs). Gate 15's skills-count check and Gate 18's registry check dropped their `design-shared` exclusion – it no longer exists. Gate 17 dropped its `ACTIVE_BRAND` check. Gate 11 dropped its now-dead `using-erfana/SKILL.md` filter, leaving `CHANGELOG.md` as the sole documented exception.
- `skills/using-erfana/SKILL.md` rewritten as an orchestration-only router: design table, design flow, brand-context section and design decision-flow branches removed; a `fact-checking` branch added.
- Documentation resynced end to end: `CLAUDE.md` (9 skills, 12 gates, brand and watermark constraints dropped, pre-commit checklist de-decked), `README.md` (design skill table and four design workflows replaced with orchestration workflows), `docs/architecture.md` (brand-system layer, per-deliverable folder convention, per-slide review and design-flow sections deleted), `docs/verification-gates.md`, the surviving `docs/gates/*.md`, `MAINTAINER.md` (deck-export and MP4 smoke checks dropped from the pre-release checklist), `SECURITY.md`, `TRADEMARKS.md`, `ROADMAP.md` (the open design / brand-system roadmap retired), `BACKLOG.md`, `docs/modernization-registry.md`, `tests/README.md`, both plugin manifests, `CITATION.cff` and the bug-report issue template.
- `managing-skills` templates, guides and the `ms-designer` / `ms-validator` agents now cite `skills/grill-me/SKILL.md` as the focused-skill reference shape instead of the deleted `design-prototype` / `design-review`. Because grill-me is a `focused` skill and not a `focused-reviewer`, the worked examples that moved with it were re-scored: `ms-validator`'s quick-validation example now reports `applicable_max: 66.5`, and `managing-skills/SKILL.md` example 5 now reports 4.5/4.5 with 12.6 marked N/A.
- Worked examples built on deleted skills were reseeded onto surviving ones: `skill-modernization-guide.md`'s 12.1 / 12.2 before-and-after blocks (was `design-slides`, now `managing-reports`), `embedded-prompts-guide.md`'s on-demand-reference example, `mi-issue-displayer`'s sample search results, and `skills/grill-me/SKILL.md`'s terminal-state handoff list.
- **Gate 18 squash-merge trap recorded.** Feature PRs squash-merge into `develop`, so the squash commit's subject becomes the touched skill's latest-commit subject there. A registry regenerated inside a skill-touching PR records the pre-squash subject and Gate 18 hard-fails the moment that PR squashes. This bit the v6.7.1 release twice – once landing on `develop`, once on the `develop` -> `main` PR. The rule that came out of it (regenerate the registry as its own registry-only change, after the skill-source PR is already on `develop`) now lives in `CLAUDE.md`; this entry is the incident record behind it.
- `docs/known-caveats.md` gains a v7.0.0 accepted-risk section (no deprecation window, no rc soak, no SVG-safety enforcement, permanently non-contiguous gate numbers) and corrects the no-soak tally, which had silently skipped v6.7.1. `docs/skill-registry.md` regenerated 15 rows -> 9. `.gitignore` lost five rules for deleted subsystems. `CONTRIBUTING.md` and `.github/pull_request_template.md` lost their brand-bundle references. `docs/assets/social-preview/card.html` documents a headless-browser regeneration route now that the bundled renderer is gone.
...
- **Generated skill registry + Gate 18.** [`docs/skill-registry.md`](docs/skill-registry.md)
  lists every shipped skill with the date and subject of the last commit that
  touched `skills/<name>/`, answering "when was each skill last worked on?"
  without re-reading the CHANGELOG. The file is generated by
  `scripts/gen-skill-registry.sh` – skill list from `ls skills/` minus
  `design-shared`, dates and subjects from `git log -1` scoped per skill
  directory – and Gate 18 (`scripts/gate-18-skill-registry.sh`) checks the
  committed table against the filesystem and git. The checks are split by what
  actually misleads a reader: a registry that lists a skill which no longer
  exists, omits one that does, lists one twice, or carries a date or subject git
  contradicts is a hard CI failure, while dates merely lagging behind git produce
  a warning with the fix command. A whole-file diff was deliberately rejected –
  the commit that changes a skill is the same commit that invalidates its row,
  and squash-merge collapses any follow-up refresh commit, so strict comparison
  would red-light `develop` after every skill change. Both scripts refuse to run
  in a shallow clone, where `git log -1 -- <path>` reports the tip commit for
  every path and would otherwise stamp all skills with one date. Release step 3
  regenerates the registry, so every shipped version is accurate, and
  `docs/skill-registry.md` joins Gate 15's `docs_to_scan` so its prose skill
  count is enforced too. Gate count moves 17 -> 18 (17 hard + 1 soft).
