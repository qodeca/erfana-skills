...
- **Every hook died after 5 ms on Qwen Code.** `hooks/hooks.json` carried `"timeout": 5` four times. Claude Code reads that field as seconds (default 600); Qwen reads it as milliseconds (default 60000), so no single value is correct on both. The key is gone and the five-second bound moved into `hooks/dispatch.sh`, where a second means a second on either host. The launcher kills the whole process **group**: hooks spawn `jq` and `grep` children that inherit stdout, and both hosts wait for the stream to close rather than for the process to exit, so a leaf-only kill bounded nothing.
- **`bash-safety` never fired on Qwen Code.** `matcher: "Bash"` does not resolve to Qwen's `run_shell_command`.
- **`secret-detector` inspected nothing on Qwen Code.** Two separate causes at two layers: the matcher was missing `write_file`, and the script's `case` fell through because the payload carries the canonical `write_file` / `edit` rather than the display name. Every Qwen write and edit reached disk unscanned.
- **Both interview guards were dead on Qwen Code.** Its extension skill parser does not extract SKILL.md `hooks:` frontmatter at all.
- **`managing-specs` was one upstream change from vanishing on Qwen.** Its `allowed-tools` used a YAML flow sequence; Qwen's strict validator throws on a non-string and the caller skips the whole skill.

### Added

- `scripts/_lib/host_matrix.py` – the one module that knows how the two hosts differ (tool aliases, the frontmatter allowlist, the timeout units, the reserved agent names). Read by two gates, the table generator and the smoke test, so the three unconverted surfaces cannot drift apart.
- [`docs/hosts.md`](docs/hosts.md) – the single source of truth other docs link to. Its host table is generated by `scripts/gen-hosts-table.sh`.
- `scripts/qwen-smoke.sh` – installs the working tree into a throwaway Qwen profile (hermetic; never touches the real `~/.qwen`) and checks the conversion. Skips cleanly without Qwen; `--require` makes that a failure.
- **PreToolUse fixtures, which never existed.** `tests/hooks/` was Stop-hook only, so nothing would have failed if the `write_file` arm were deleted again. Each family now has Claude-shaped and Qwen-shaped variants, and Gate 16 proves the launcher's behaviour rather than the presence of a string: a hook that wedges on purpose must be killed inside ten seconds, and with `jq` masked off `PATH` the parsing hooks must skip with a diagnostic while `post-compact-reminder`, which needs no `jq`, must still run.
- **Ten new gate checks.** Gate 14 gains the timeout ban, matcher dual-naming read from `host_matrix.py`, a plain-matcher rule, a literal-block-message rule and a launcher check. Gate 2 gains the Qwen agent-name rules, string-typed `allowed-tools` / `argument-hint`, and the `disable-model-invocation` constraint on `fact-checking` that `CLAUDE.md` had always stated and nothing enforced. Gate 15 gains two scanned docs and a Qwen version-agreement check.
- **Three CI jobs**: `qwen-compat` (pinned Qwen 0.22.3), `powershell` on windows-latest – the only place the `.ps1` hooks actually execute – and a weekly `qwen-canary` against `@latest` that never blocks and files one tracking issue. Plus `.gitattributes`, without which the Windows job reds immediately on CRLF checkouts.

### Changed

- **The two interview guards moved** from SKILL.md `hooks:` frontmatter into `hooks/hooks.json`, and their scripts from `skills/*/hooks/` to `hooks/`. The gated hook count goes 4 to 6. They now evaluate every stop, scoped by the open-marker sentinel alone – recorded as an accepted risk in [`docs/known-caveats.md`](docs/known-caveats.md), together with the four false-block shapes found in review and fixed with fixtures.
- **Agent bodies name the action, not the tool.** About 202 fake tool calls across 32 files used Claude Code's tool syntax, which means nothing on Qwen. Real shell commands are kept verbatim with only the wrapper dropped. Frontmatter `tools:` lists are untouched – those are the grant, and Qwen's converter remaps them correctly.
- **The three argument-taking commands resolve their arguments per host.** `$ARGUMENTS` is literal on Qwen, `{{args}}` is literal on Claude Code. The resolution rule deliberately names neither token, because each host would substitute its own inside the rule that mentions it.
- `project-status` and `session-status` gained a `description` so Qwen stops rendering them as "Custom command from ...", plus `disable-model-invocation: true` – the flag is load-bearing, since a `description` alone would newly expose both to Claude Code's SlashCommand tool.
- The three Anthropic-only model rules (`temperature` / `top_p` / `top_k`, fixed `thinking` budgets, reasoning display) are now scoped as Anthropic-only in the guides. The reasoning-display ban stays, with its cause stated: an Anthropic refusal classifier.

### Known limitations

No erfana skill has yet run end to end inside a Qwen session. Everything here verifies the loader and the conversion, not the executor. See [`docs/known-caveats.md`](docs/known-caveats.md) for the full list, including the `jq` and watchdog fail-open directions and the unresolved question of what `model: opus` does on Qwen.
