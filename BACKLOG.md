...
## Deferred to a future major – concrete-trigger entries (added v4.2.0)
...
## Deferred — concrete-trigger entries (added v6.6.1)

- **Gate-count claim phrasings are almost entirely unenforced.** Gate 15 check 7 guards exactly two forms – `N per-gate detail files` and `gates/01-N` – deliberately narrowed so it cannot match generic prose. The Gate 18 release surfaced how thin that cover is: of eleven current-state gate-count claims in the repo, only two sit inside the enforced patterns. The nine carrying the load are `all N gates (N hard + N soft)` (`CLAUDE.md`, `docs/verification-gates.md` twice, `docs/architecture.md` twice), the spelled-out `Eighteen static checks` opener, the gate index table, and the runner-order line – all correct today, all free to rot on the next gate addition. **Trigger:** adding Gate 19, or the first time one of these is found stale. **Why deferred:** widening the regexes risks matching the historical claims in `CHANGELOG.md`, `ROADMAP.md`, `docs/known-caveats.md` and `docs/release-notes-v6.0.0.md`, which are correct-for-their-release and must not be "fixed" – the narrowing was a deliberate choice, not an oversight, and undoing it needs a per-file exemption model rather than a bigger pattern. **Cheapest safe increment:** count the runner's gate invocations and compare against the index table's row count, rather than pattern-matching prose.

- **The skill registry is not surfaced outside maintainer docs.** `docs/skill-registry.md` is linked from `CLAUDE.md`, `docs/architecture.md`, `docs/verification-gates.md` and `CONTRIBUTING.md`, but not from `README.md` (no docs-inventory section exists to hold it) or `MAINTAINER.md`'s knowledge-silo list, and `docs/verification-gates.md` `## Quick spot-checks` has no Gate 18 entry. **Trigger:** backup-maintainer onboarding (the knowledge-silo list is read then), or a user asking where to see per-skill freshness. **Why deferred:** the registry is maintainer-facing tooling and `README.md` is the end-user onboarding surface; adding it there without a docs index would be the first item of a section that does not exist. Note the spot-checks gap is pre-existing and not specific to Gate 18 – gates 12, 15, 16 and 17 have no entry either.

- **Registry row rendering is mirrored between two scripts instead of shared.** `gen-skill-registry.sh` and `gate-18-skill-registry.sh` each implement the same subject truncation (72 chars) and pipe escaping; a comment in both files is the only thing keeping them in step, and drift makes the gate reject rows the generator legitimately emits. Recorded as an accepted risk in [`docs/known-caveats.md`](docs/known-caveats.md); this is the work item to close it. **Trigger:** the first time the rendering rule actually changes. **Why deferred:** the repo has no shared-library convention for gate scripts (`scripts/_lib/` holds data, not code), so factoring this out means inventing that convention for a twelve-line function – disproportionate until a second consumer exists.

## Deferred — interrogation follow-ups (added v6.4.0)

- **Sibling adoption of the coverage-map interview.** `managing-agents`, `managing-issues`, and `managing-specs` adopt `grill-planner` by shipping their own `interview-taxonomy.md` + `interview-protocol.md` copies, their own sentinels (lane convention: `<!-- erfana:ma-grill-open -->`, `<!-- erfana:mi-grill-open -->`, ...), and their own skill-scoped guard hooks (Gate 16 family each). The shared agent needs no edits — that is the point of the `taxonomy_path` input. **Trigger:** first maintenance touch on a sibling's requirements gathering, or a dedicated cascade release. **Status note (2026-08-04):** the trigger fired on `managing-issues` — the Implement-operation hardening edited `phases/2-business-analysis.md`, and `operations/implement.md` now states that the Phase 2 requirements questionnaire blocks on both tiers. Adoption was deliberately **not** taken: that change set was scoped to making the Implement gates binding, not to requirements gathering, and pulling in a taxonomy + protocol + guard hook would have widened it well past review. The trigger re-arms on the next touch of a sibling's requirements gathering.
- **Post-rollout interrogation metrics.** Verification of the feature's actual value: track (a) rework passes per Create (follow-up Modify operations needed to match intent — observable in the modernization registry and git history) and (b) first-pass `ms-validator` threshold rate. **Status note (v6.4.0):** the interview shipped in v6.4.0 without a dedicated baseline capture; the pre-change baseline is the pre-v6.4.0 git/registry history of create-then-modify sequences. **Trigger:** first review of the interrogation feature's value (e.g. before the sibling cascade).
