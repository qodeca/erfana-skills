...
> **Staged-rollout override tracking moved out of this file after v4.2.13.** Per-release admin-merge / no-rc-soak overrides are now recorded only in [`docs/known-caveats.md`](docs/known-caveats.md). The "fifth admin-merge override" counter in the v4.2.13 section below is historical – accurate when written, not maintained here since.
...
Tracked in `BACKLOG.md` under "Deferred to a future major – concrete-trigger entries". The cascade plan will be authored separately when the trigger materializes.
