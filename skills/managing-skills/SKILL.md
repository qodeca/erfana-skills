...
20. **On Anthropic models, skills MUST NOT use deprecated APIs** (added v4.2.0; provenance scoped v7.1.0): no `temperature` / `top_p` / `top_k` (400 error on Claude Opus 4.7 and later, per Anthropic's parameter-deprecation table) and no fixed `thinking: {budget_tokens: N}` on Claude 5 models (unsupported on Fable 5 / Opus 5 / Sonnet 5; Haiku 4.5 still supports it). Use `{type: "adaptive"}` + `effort` field instead. Other model families prescribe the opposite - Qwen's own model card asks for `temperature=1.0, top_p=0.95, top_k=20` - so this is a constraint of the target, not a truth about prompting. It stays ⛔ BLOCKING for what this plugin ships, because what this plugin ships runs on Anthropic models (Section 12.7 of pre-release-checklist; `guides/cross-model-guide.md`).
