 ...
 *
 * Stage + Sprite pattern, inspired by Remotion but lighter.
 *
 * Exports (attached to window.Animations):
 * - Stage: the whole animation container, provides time + control
 * - Sprite: time segment, displayed within start/end, provides local progress
 ...
 *   </Stage>
 *
 * Inside Sprite child components, use useSprite() to read the current segment progress.
 */
...
(function() {
  ...
  const Easing = {
    ...
    // expoOut: the Anthropic-level primary easing (cubic-bezier(0.16, 1, 0.3, 1))
    // quick start + slow brake, gives digital elements a sense of physical weight
    expoOut: t => t === 1 ? 1 : 1 - Math.pow(2, -10 * t),
    // overshoot: spring-loaded toggle/button pop (cubic-bezier(0.34, 1.56, 0.64, 1))
    overshoot: t => {
