...
(async () => {
  ...
  if (hasReady) {
    // Second line of defense: proactively reset the animation's time to zero — handles
    // HTML that doesn't strictly follow the starter tick template (e.g. lastTick using
    // performance.now() causes font-load time to be counted into the first-frame dt).
    // See references/animation-pitfalls.md section 12.
    ...
    if (seekCorrected) {
      // Wait for two rAF frames so the seek takes effect and the t=0 frame renders
