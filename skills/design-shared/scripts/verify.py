...
Usage:
    ...
    python verify.py deck.html --slides 10                      # Slide deck per-page screenshots (first 10)
...
def verify_html(html_path, viewports=None, slides=0, output_dir=None, show=False, wait=2000):
        ...
        for viewport in viewports:
            ...
            if slides > 0:
                for i in range(slides):
                    ...
                    print(f"  OK slide {i+1} -> {screenshot_path.name}")
    ...
    if page_errors:
        print(f"\n[FAIL] Page Errors ({len(page_errors)}):")
    ...
    if console_errors:
        print(f"\n[WARN] Console Errors/Warnings ({len(console_errors)}):")
        for e in console_errors[:20]:
            print(f"  - {e}")
        if len(console_errors) > 20:
            print(f"  ... and {len(console_errors) - 20} more")
    else:
        print("[OK] Console is clean")
...
def main():
    ...
    parser.add_argument("--viewports", default="1440x900",
                        help="Comma-separated viewport list in WxH format (default 1440x900)")
    parser.add_argument("--slides", type=int, default=0,
                        help="Slide mode: capture first N slides (HTML must support ArrowRight navigation)")
    parser.add_argument("--output", default=None,
                        help="Output directory (defaults to a screenshots/ folder next to the HTML)")
