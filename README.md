...
| `erfana:managing-issues` | Full GitHub-issue lifecycle: create issues, implement them through phased quality gates, and review code or PRs. Runs on `main`-default repos, stack-agnostic, with a skill-wide untrusted-data boundary and confirm-before-destructive git ops. The Implement operation is **interactive-only** – it refuses a headless run and stops for your approval at several gates; at two of those checkpoints it hands the run back to you entirely, printing an `/erfana:lens-review` command and ending its turn, then waiting until you have run that review yourself in a separate session and returned the report path. It saves its progress to one comment on the issue (after asking, on a public repo; without a prompt but announced in one line, on a private one) so an interrupted run can resume. See [`docs/architecture.md`](docs/architecture.md). | "create issue", "implement issue", "review code", "review PR" |
